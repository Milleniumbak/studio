<?php

namespace app\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\Susuario;

/**
 * SusuarioSearch represents the model behind the search form about `app\models\Susuario`.
 */
class SusuarioSearch extends Susuario
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['pkusuario', 'stipousuario'], 'integer'],
            [['snombre', 'sapellido', 'semail', 'stelefono', 'spassword', 'sestado'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Susuario::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'pkusuario' => $this->pkusuario,
            'stipousuario' => $this->stipousuario,
        ]);

        $query->andFilterWhere(['like', 'snombre', $this->snombre])
            ->andFilterWhere(['like', 'sapellido', $this->sapellido])
            ->andFilterWhere(['like', 'semail', $this->semail])
            ->andFilterWhere(['like', 'stelefono', $this->stelefono])
            ->andFilterWhere(['like', 'spassword', $this->spassword])
            ->andFilterWhere(['like', 'sestado', $this->sestado]);

        return $dataProvider;
    }
}
